...
key="65a7b9e7b5204eff8ca880e216cdc2b4"


def Entry_domain():
    domain = str(input_domain.get())
    response = requests.get(
        url=f"https://api.whoisfreaks.com/v1.0/whois?apiKey={key}&whois=live&domainName={domain}")
    res = response.json()
...
# lable for text show
label1 = Label(text="Useing Tkinter", font=(
    "Arial", 30, "bold"), background="#4d76a3")
label1.grid(column=2, row=0)

# lable for text show
label2 = Label(text="Domain Name", font=(
    "Sans Serif", 22, "bold"), background="#4d76a3")
